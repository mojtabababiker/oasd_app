...
import { fetchPost, fetchRelatedPosts, getFeaturedPosts } from "@/app/utils/scripts/posts_api";
...
export default async function PostPage(params: { params: { slug: string }, searchParams: { string: any } }) {
  ...
  const slugTitle = slug.split('-').map(word => { return (word.charAt(0).toUpperCase() + word.slice(1)) }).join(' ');
  ...
  const breadcrumb = {
    ...
    ancestors: [
      ...
      { title: post.category, link: `/posts?category=${post.category.toLowerCase()}` }
    ]
  };
  // console.log(params);
  ...
  return (
    <>
      <BreadCrumb self={breadcrumb.self} ancestors={breadcrumb.ancestors} bgImage={`url(${post.thumbnail})`} />
      <section className="container-fluid section post-main">
        ...
        <aside className="related-posts-aside">
          ...
          <div className="related-posts-container">
            ...
            <Button href={`/posts?${post.category}`} className='solid-btn-primary'>See More</Button>
          </div>
        </aside>
      </section>
      {/* featured post */}
